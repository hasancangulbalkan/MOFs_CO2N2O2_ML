...
tpot_data = pd.read_excel('CO2_1BAR.xlsx', dtype=np.float64)
features = tpot_data.drop('CO2 1 bar (mol/kg)', axis=1)
training_features, testing_features, training_target, testing_target = \
            train_test_split(features, tpot_data['CO2 1 bar (mol/kg)'],train_size=0.80, test_size=0.20, random_state=42)

# Average CV score on the training set was: -0.24661527748904982
exported_pipeline = XGBRegressor(learning_rate=0.1, max_depth=7, min_child_weight=5, n_estimators=100, n_jobs=1, objective="reg:squarederror", subsample=0.45, verbosity=0)
